...
class ResilienceEngine:
    ...
    async def handle_container_die(self, container_id, name, image):
        # ── CRITICAL CHECK — only failover containers marked as critical ───────
        # Without this, every container die (cache nodes, standby, infra)
        # triggers a replica — flooding Docker with unwanted containers.
        
        #case 1
        # if name.endswith(tuple([f"-replica-{container_id[:6]}"])) or \
        #     "cache-node" in name or "standby" in name:
        #case 2
        ...
        try:
            ...
            container = await loop.run_in_executor(
                ...
                lambda: self.docker_client.containers.create(
                    image=image, name=replica_name, detach=True
                ),
            )
            ...
            audit_entry = {
                ...
                "original_container": original_id,
            ...
            self.state_store.upsert_container(
                container.id[:12],
                {
                    ...
                    "name":         replica_name,
                    "image":        image,
                    "status":       "running" if recovered else "unknown",
                    "is_replica":   True,
                    "original_id":  original_id,
                },
            )
            # Keep replica marked critical so IT also gets a replica if it dies
            self.state_store.mark_critical(container.id[:12])

            # Broadcast auto_failover event — dashboard increments counter + shows replica
            await self.ws_manager.broadcast(audit_entry)

        except DockerException as exc:
            logger.error(f"Failover failed for {name}: {exc}")
            self.state_store.append_audit({
                "event_type":         "auto_failover_error",
                "original_container": original_id,
                "error":              str(exc),
                "timestamp":          datetime.utcnow().isoformat(),
            })
