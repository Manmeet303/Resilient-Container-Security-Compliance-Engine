...
class ResilienceEngine:
    ...
    async def handle_container_die(self, container_id, name, image):
        logger.info(f"Container {name} stopped/died — initiating auto-failover.")
    ...
    async def _failover(self, original_id, name, image):
        ...
        replica_name = f"{name}-replica-{datetime.utcnow().strftime('%H%M%S')}"
        ...
        try:
            ...
            # Optional: keep replica marked critical
