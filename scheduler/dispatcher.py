...
class Dispatcher:
    ...
    async def dispatch_loop(self):
        while True:
            ...
            # NEW:
            # This increases load, jobs assigned, and events assigned.
            self.registry.mark_assigned(worker_id)
            ...
            # Immediately update dashboard so professor can see task assigned.
            try:
                await worker_node._report_to_control_plane()
            except Exception:
                pass
        ...
        try:
            ...
            try:
                async with httpx.AsyncClient() as cache_http:
                    ...
                    logger.info(
                        f"Cache write: {cache_resp.status_code} "
                        f"| hash={layer_hash[:16]}"
                    )
            ...
            # NEW:
            # This increases completed count and decreases current load.
            self.registry.mark_completed(worker_id)

            # Keep local worker counter in sync.
            worker_node.jobs_completed = self.registry.workers[worker_id].get(
                "jobs_completed",
                0,
            )
            ...
            # Notify control plane so Live Events and scan result update.
            ...
            logger.error(
                f"Worker {worker_id[:8]} failed job {job['job_id'][:8]}: {exc}"
            )

            # Job failed, so load should go down but completed should NOT increase.
            self.registry.mark_failed_or_requeued(worker_id)
            ...
            # Safety guard: never allow load below 0.
            if worker_id in self.registry.workers:
                current_load = self.registry.workers[worker_id].get("load", 0)
                self.registry.update_load(worker_id, max(0, current_load))
    ...
    async def _notify_scan_complete(self, result: dict, job: dict):
        try:
            ...
            logger.warning(
                f"Could not notify control plane of scan complete: {exc}"
            )