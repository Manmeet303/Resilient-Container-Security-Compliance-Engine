...
- **Role:** The Brain.
- **Function:** Hooks into the Docker Engine API via socket to listen for `container_start` and `container_die` events. It pushes state updates to a real-time WebSocket dashboard and executes the Auto-Failover logic (instantly spinning up replicas when critical containers fail).
...
1. Clone the repository:
   ```bash
   git clone https://github.com/your-org/resilient-container-security.git
````
